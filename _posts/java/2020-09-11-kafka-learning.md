...
## 客户端监控
所有的客户端都需要监控，不管是生产者还是消费者。
### 生产者度量指标
- 生产者整体度量指标，record-error-rate、record-retry-rate、request-latency-avg、outgoing-byte-rate、record-send-rate、request-rate、request-size-avg、batch-size-avg、records-per-request-avg、request-queue-time-avg。
- Per-broker和Per-topic度量指标，request-latency-avg、outgoing-byte-rate、record-send-rate、record-error-rate、byte-rate
### 消费者度量指标
- Fetch Manager度量指标，fetch-latency-avg、fetch.min.bytes、fetch.max.wait.ms、bytes-consumed-rate、records-consumed-rate、fetch-rate、fetch-size-avg、records-per-request-avg
- Per-broker和Per-topic度量指标，request-latency-avg、incoming-byte-rate、request-rate
- Coordinator度量指标，sync-time-avg、sync-rate、commit-latency-avg、assigned-partitions。
### 配额
Kafka可以对客户端请求进行限流，防止客户端拖垮整个集群。可配置每秒钟允许单个客户端访问单个broker的流量字节数来表示。默认情况下，broker不会开启配额功能。
## 延时监控
对于消费者来说，最需要被监控的指标就是消费者的延时。它表示分区最后一个消息和消费者最后读取的消息之间相差的消息个数。监控延时的最好办法就是使用外部进程(如：Burrow)，他能够观察broker的分区状态，跟踪最近消息的偏移量，也能观察消费者的状态。
## 端到端监控
使用Kafka Monitor实现外部监控系统的端到端的监控，Kafka Monitor持续向一个横跨集群所有broker的主题生成消息，并读取这些消息。
# 流式处理
Kafka一般被认为是一个强大的消息总线，可以传递事件流，但没有处理和转换事件的能力。Kafka可靠的传递能力让它成为流式处理系统完美的数据源。很多基于kafka构建的流式处理系统都将它作为唯一可靠的数据来源。如：Apache Storm、Apache Spark Streaming、Apache Flink、Apache Samza等。
## 什么是流式处理
数据流，也被称为事件流，或流数据。数据流是无边界数据集的抽象表示，无边界意味着无限和持续增长。之所以是无限的，是因为随着时间推移，新的记录会不断进来。除了无限外，数据流还是有序的、不可变的、可重播的。
流式处理是指实时处理一个或多个数据流。流式处理是一种编程范式，就像请求与响应范式和批处理范式那样。以下是对三者的对比：
|  范式    | 延时     |  是否阻塞    |举例 |
| ---- | ---- | ---- | ---- |
|请求与响应|   小,亚毫秒到毫秒   |  阻塞    | 线上交易处理(OLTP)、销售点(POS)系统、信用卡处理系统、基于时间的追踪系统 |
| 批处理  |  高延时，高吞吐量    |  非阻塞    | 数据仓库(DWI)、商业智能(BI)|
|流式处理 | 延时居中     | 持续非阻塞  | 针对信用卡交易警告、网络警告、根据供应关系实时调整价格、跟踪包裹 |
## 一些概念
- 时间，分为事件时间，包括事件发生时间和事件记录时间。日志追加时间，指日志被保存到broker的时间。处理时间，指应用程序在收到事件之后要对其进行处理的时间。
- 状态，事件与事件之间的信息被称为状态，这些状态一般被保存在应用程序本地变量里。流式处理包括以下几个类型的状态:1.本地状态或内部状态，这种状态只能被单个应用访问，一般使用内嵌在应用程序里的数据库进行维护和管理。本地状态的优势在于速度，不足之处在于受到内存大小限制。2.外部状态，这种状态使用外部数据存储来维护，一般使用NoSQL系统，比如Cassandra，优势在于没有大小限制，而且可以被多个应用程序实例访问，甚至被不同的应用程序访问。不足之处在于，引入额外的系统会造成更大的延迟和复杂性。
- 流和表的二元性，表转化为流，需要捕捉表上发生的变更，将事件保存到流中。流转化为表，需要“应用”流里所包含的所有变更，也叫做流的“物化”。
- 时间窗口，分为滚动窗口和跳跃窗口。
## 流式处理的设计模式
### 单个事件处理
处理单个事件是流式处理最基本的模式。这个模式也叫map或filter模式，经常被用于转换事件。在这种模式下，应用程序读取流中的事件，修改他们，然后把事件生成到另一个流上。
### 使用本地状态
大部分流式处理应用程序关心的是如何聚合消息，特别是基于时间窗口进行聚合。要实现这些聚合，需要维护流状态到本地来实现。
### 多阶段处理和重分区
本地状态对按组聚合操作起到很大作用，但如果需要所有可用的信息来获得一个结果时，可拆分阶段，使用不同阶段统计不同信息。
### 使用外部查找-流和表连接
流式处理和外部数据集成在一起，完成计算。
### 流与流的链接
将两个流里具有相同键和发生在相同时间窗口内的事件匹配起来。这种也叫基于时间窗口的连接。
### 乱序的事件
